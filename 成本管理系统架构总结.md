...
**需求**：打开 Lookup 弹窗时，如果当前字段已有值，需要：
1. 根据 VALUE_FIELD 的当前值查询对应数据
2. 在弹窗表格中自动定位并选中该行
3. 如果数据在当前页之外，需要跳转到对应页或通过搜索定位

**实现方案**：
- 弹窗打开时，读取当前单元格的 valueField 值
- 调用 Lookup 数据源接口，传入 valueField 作为精确查询条件
- 获取到数据后，设置 AG Grid 的选中状态并滚动到该行
