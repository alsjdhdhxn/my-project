...
### 1.9 数据源支持 ✅
- V3 主表强制使用 Server-Side Row Model (SSRM)
- V3 明细表强制使用 Client-Side Row Model
- 分页、排序、过滤条件构建
- SSRM 数据源自动挂载（grid-ready 时设置 datasource）
- SSRM 模式过滤刷新策略（refreshServerSide）
...
- POST /page/{pageCode}/execute - 执行页面规则中的Action（BUTTON）
...
- ROW_CLASS - 行级样式规则（支持 className 和内联 style）
- CELL_EDITABLE - 单元格级可编辑规则

### 3.4 页面组件（T_COST_PAGE_COMPONENT） ✅
- LAYOUT - 布局容器
- GRID - 数据表格
- TABS - 明细标签页
- FORM - 动态表单（有渲染器，暂无配置数据）
- BUTTON - 按钮（有渲染器，暂无配置数据）
- MASTER_DETAIL - 主从布局（有渲染器，暂无配置数据）
- LOGIC_AGG - 聚合逻辑（预留）
- LOGIC_FIELD - 字段联动（预留）
