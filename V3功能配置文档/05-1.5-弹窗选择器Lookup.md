...
| 字段 | 类型 | 必填 | 说明 |
|------|------|------|------|
| field | string | 是 | 触发 lookup 的字段名 |
| lookupCode | string | 是 | 对应 T_COST_LOOKUP_CONFIG 的 LOOKUP_CODE |
| mapping | object | 是 | 回填映射，key=目标字段，value=弹窗字段 |
| noFillback | boolean | 否 | 是否禁止回填（仅查看模式），默认 false |

### noFillback 说明
当 `noFillback: true` 时：
- 即使行不可编辑（如 ISERP=1），也允许打开弹窗查看数据
- 选择数据后不会回填到表格，仅供查看用途
- 适用于需要查看关联数据但不允许修改的场景

```json
{
  "field": "goodsid",
  "lookupCode": "goodsByApex",
  "mapping": { "goodsid": "pGoodsid", "goodsname": "goodsname" },
  "noFillback": true
}
```
