...
??????? `T_COST_TABLE_METADATA.VALIDATION_RULES`??????? `T_COST_TABLE_METADATA.ACTION_RULES`??????
...
**?????**?`result == 0`, `result != 0`, `result > 0`, `result >= 1`, `result < 10`
...
[
  ...
  },
  {
    ...
    "group": "manual",
    "type": "proc",
    "procedure": "P_SYNC_COST",
    "params": [
      { "name": "P_ID", "mode": "IN", "jdbcType": "NUMERIC", "source": "data.id" },
      { "name": "P_MSG", "mode": "OUT", "jdbcType": "VARCHAR", "target": "vars.procMsg" }
    ]
  }
]
```
...
- `type`: `sql` / `java` / `proc`
- `enabled`: ??????? true?
- `handler`: `java` ??? Bean ?????? ActionHandler?
- `method`: `java` ??????? `beanName.methodName`
- `sql`: type=sql ???
- `procedure`: type=proc ???
- `params`: type=proc ??????? `IN` / `OUT` / `INOUT`
- `source`: `data.<??>` ? `vars.<??>`
- `target`: `vars.<??>` ? `data.<??>`
- `description`: ??

Java ?????`ActionResult method(ActionContext ctx)`????????????
???`handler` ? `method` ??????
