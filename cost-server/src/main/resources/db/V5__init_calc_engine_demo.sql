...
BEGIN
    ...
    '{"style":[{"condition":{"type":"contains","pattern":"瓶"},"style":{"color":"red"}}]}', 'system');
    INSERT INTO T_COST_COLUMN_METADATA (ID, TABLE_METADATA_ID, FIELD_NAME, COLUMN_NAME, HEADER_TEXT, DATA_TYPE, DISPLAY_ORDER, EDITABLE, WIDTH, CREATE_BY) 
...
BEGIN
    ...
    -- 第三条测试数据（产品名称包含"瓶"，测试样式规则）
    DECLARE
        v_eval_id3 NUMBER;
    BEGIN
        SELECT SEQ_COST_EVAL.NEXTVAL INTO v_eval_id3 FROM DUAL;
        
        INSERT INTO T_COST_EVAL (ID, EVAL_NO, PRODUCT_NAME, APEX_PL, YIELD, OUT_PRICE_RMB, CREATE_BY)
        VALUES (v_eval_id3, 'EVAL-2025-003', '阿莫西林口服液瓶装', 50, 96, 28.0, 'system');
    END;
